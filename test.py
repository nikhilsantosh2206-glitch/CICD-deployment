...
dataset = pd.read_csv(r'C:\CICD\iris.csv')
dataset.head()
...
#Length Check
sepal_length_test = dataset['sepal length (cm)'].between(4.0,8.0).all()
sepal_width_test = dataset['sepal width (cm)'].between(2,5).all()
petal_length_test = dataset['petal length (cm)'].between(1.0,7.0).all()
